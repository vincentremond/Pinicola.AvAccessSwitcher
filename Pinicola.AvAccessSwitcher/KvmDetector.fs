...
module KvmDetector =
    ...
    let isKvmConnected () : bool =
        try
            use searcher = new ManagementObjectSearcher(@"SELECT DeviceID, Status, Name FROM Win32_PnPEntity WHERE Status = 'OK'")
        ...
        with ex ->
            eprintfn "Error querying KVM status: %s" ex.Message
            false
